...
const ProductScreen = () => {
	...
	const addItem = () => {
		const itemToAdd = { ...product, price: product.hasDiscount ? calculateDiscountedPrice(product.price, product.discountPercentage) : product.price };
		dispatch(addCartItem(id, amount, itemToAdd));
		toast({
	...
	const ImageGallery = ({ images }) => {
		...
		while (allImages.length < 5) {
			allImages.push('https://via.placeholder.com/250');
		}
				...
				<Flex gap="2" justify="center" w="full">
					{allImages.map((image, index) => (
						<Box
							...
							cursor="pointer"
							border="2px"
							borderColor={selectedImage === index ? 'cyan.500' : 'transparent'}
					...
					>
						<Stack direction={{ base: 'column', lg: 'row' }} align="flex-start">
							<Stack pr={{ base: '0', md: 'row' }} flex="1.5" mb={{ base: '12', md: 'none' }}>
								...
								{product.hasDiscount && (
									<Badge w="70px" fontSize="0.8em" colorScheme="red">
